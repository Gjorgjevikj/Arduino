...
// PUSH_BUTTON_L(obj_name, push_button_pin, ptr_to_int_function_to_be_called);
// SET_PB(obj_name, push_button_pin, ptr_to_int_function_to_be_called, user_ISR_finction, type_of_button, debounce_duration);
PUSH_BUTTON_L(button1, PB2, FlashLeds);
//SET_PB(button1, PB2, FlashLeds, MonitorChange1, LOW, 50);
...
void loop() {
  ...
  // you can put any other code to monitor other sensors / perform other tasks

  // Just monitoring the current state od the pins 2 and 3 (push button 1 & 2 are connected to)
  // the functions will fire up on button presses and will print on the serial that will appear in COM monitor
  ...
  Serial.print("  ");
  Serial.print(button1.Pending());
  Serial.print(" : ");
  Serial.print(button2.Pending());
  Serial.println();
