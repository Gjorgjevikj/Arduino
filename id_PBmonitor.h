...
class PBmonitor // the class represntin a push button to be monitored using interrupts
{
  public:
    PBmonitor(int p, PBcallback f, ISR isrv, bool act = HIGH, unsigned long t_i = 20) : 
      pin(p), isr(isrv), callback(f), active(act), t_ignore(t_i), e_millis(0), inCallback(false), pstate(act?LOW:HIGH), monitoring(false) 
      { /* assert(digitalPinToInterrupt(p) != NOT_AN_INTERRUPT); */ }
    ...
    { 
      ...
      pstate=state;
      if(push_registered && now-e_millis > t_ignore) // was pressed long enought
      {
        if(!inCallback) 
        {
          //detachInterrupt(digitalPinToInterrupt(pin)); // disable monitoring the buttin while in service
          //monitoring=false;
          inCallback=true;
          interrupts();
          callback(now-e_millis);
          noInterrupts();
          inCallback=false;
          //monitoring=true;
          //attachInterrupt(digitalPinToInterrupt(pin), isr, CHANGE); // set interrupt on change again
        }
      }
    }
  ...
  private:
    ...
    bool pstate; // previous state of the pin (checked in the ISR)
    unsigned long e_millis; // elapsed millis since the last call to ISR
